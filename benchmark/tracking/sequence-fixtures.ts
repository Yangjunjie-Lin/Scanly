...
function buildScenario(options: ScenarioOptions): TrackingSequenceScenario {
  ...
  const observationFrames: TrackingObservationFrame[] = Array.from({ length: frameCount }, (_, frameIndex) => {
    const observations = options.emptyObservationFrames?.includes(frameIndex)
      ? []
...
export const TRACKING_SEQUENCE_SCENARIOS: readonly TrackingSequenceScenario[] = [
  ...
  buildScenario({
    ...
    expected: { minimumTrackRecall: 0.75 }, trackerOptions: { lostGraceFrames: 5, retirementFrames: 9 },
  }),
  buildScenario({
    ...
    expected: { minimumConfirmedTrackCount: 2, maximumConfirmedTrackCount: 2, maximumIdentitySwitchCount: 1, maximumTrackFragmentationCount: 1, minimumTrackRecall: 0.9 },
    trackerOptions: { lostGraceFrames: 2, retirementFrames: 5 },
  }),
  buildScenario({
  ...
  buildScenario({
    ...
    expected: { minimumTrackRecall: 0.8 }, trackerOptions: { lostGraceFrames: 3 },
  }),
  buildScenario({
  ...
  buildScenario({
    ...
    batch: { mode: "checklist", expected: [{ payload: "ABC-001", format: "code_128" }, { payload: "ABC-002", format: "code_128" }, { payload: "XYZ", format: "data_matrix" }], status: "complete", matchedQuantity: 3, missingQuantity: 0, unexpectedQuantity: 0, duplicateQuantity: 0 },
  }),
  buildScenario({
    id: "batch-duplicate-quantity", category: "batch", description: "Three equal UPC physical instances satisfy quantity three without payload Set deduplication.",
    objects: [object("quantity-a", SAME_UPC, linear(50, 200), { format: "upc_a" }), object("quantity-b", SAME_UPC, linear(250, 200), { format: "upc_a" }), object("quantity-c", SAME_UPC, linear(450, 200), { format: "upc_a" })],
    expected: { samePayloadInstanceSeparation: true },
    batch: { mode: "checklist", expected: [{ payload: SAME_UPC, format: "upc_a", quantity: 3 }], status: "complete", matchedQuantity: 3, missingQuantity: 0, unexpectedQuantity: 0, duplicateQuantity: 0 },
  }),
  buildScenario({
    ...
    batch: { mode: "checklist", expected: [{ payload: "EXPECTED", format: "code_128" }], status: "complete", matchedQuantity: 1, missingQuantity: 0, unexpectedQuantity: 1, duplicateQuantity: 0 },
  }),
  buildScenario({
    ...
    batch: { mode: "checklist", expected: [{ payload: "PRESENT", format: "code_128" }, { payload: "MISSING", format: "code_128" }], status: "collecting", matchedQuantity: 1, missingQuantity: 1, unexpectedQuantity: 0, duplicateQuantity: 0 },
  }),
  buildScenario({
  ...
  buildScenario({
    ...
    objects: gridObjects(8, true, "DROP"), emptyObservationFrames: [4, 5, 11], expected: { minimumTrackRecall: 0.8 }, trackerOptions: { lostGraceFrames: 4 },
  }),
  buildScenario({
    ...
    expected: { minimumTrackRecall: 0.85 }, trackerOptions: { lostGraceFrames: 3 },
  }),
  buildScenario({
...
export const REQUIRED_TRACKING_SCENARIO_IDS = TRACKING_SEQUENCE_SCENARIOS.map(({ id }) => id);

if (TRACKING_SEQUENCE_SCENARIOS.length < 24) {
  throw new Error(`Tracking evidence requires at least 24 scenarios; found ${TRACKING_SEQUENCE_SCENARIOS.length}.`);
}
