...
function emptyStatus(): Json {
  return {
    ...
    physicalValidationStatus: "PHYSICAL_DEVICE_VALIDATION_PENDING",
...
describe.sequential("Beta 4 device evidence contracts", () => {
  ...
  it("keeps the Foundation gate pending when every physical session leaves permission untested", () => {
    ...
    const status = { ...statusForFoundationFixtures(), physicalValidationStatus: "PHYSICAL_DEVICE_VALIDATION_PENDING", requiredGaps: [foundationGaps[3], ...fullMatrixGaps] };
    withTemporaryEvidenceSet([android, ios], status, () => expect(runVerifier()).toContain("PHYSICAL_DEVICE_VALIDATION_PENDING"));
  }, 30_000);
