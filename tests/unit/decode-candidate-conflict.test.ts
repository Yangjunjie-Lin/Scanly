...
function candidate(payload: string, route: RecoveryRouteId, options: { checksum?: boolean; x?: number } = {}): DecodeCandidate {
  const x = options.x ?? 0;
  const result: ScanResult = {
    format: "qr_code", rawText: payload, cornerPoints: [{ x, y: 0 }, { x: x + 20, y: 0 }, { x: x + 20, y: 20 }, { x, y: 20 }],
  ...
  return { payload, format: "qr_code", engine: "fake", route, geometry: result.cornerPoints, validation: { decoderValidation: true, formatStructuralValidity: true, ...(options.checksum === undefined ? {} : { checksumValidated: options.checksum }) }, result, elapsedMs: 1 };
}
