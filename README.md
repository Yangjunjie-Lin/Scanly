...
- RC1 automated status is reported by exact-source scanner, tracking, industrial, browser, native, API/ABI, security, artifact, and reproducibility workflows. `NATIVE_PHYSICAL_IOS_PENDING` and `NATIVE_PHYSICAL_ANDROID_PENDING` remain explicit until the final RC campaign.
