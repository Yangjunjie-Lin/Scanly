import type { BenchmarkRunSummary } from "../qr/benchmark-types";
...
const HARD_ATTEMPT_LIMITS: Record<string, number> = {
  "14-damaged": 110,
...
export function evaluateBenchmarkGates(
  ...
  if (options.fullSuite) {
    ...
    if (summary.successRate + Number.EPSILON < minimumRate) {
      failures.push(
        `success rate ${(summary.successRate * 100).toFixed(2)}% is below ${(minimumRate * 100).toFixed(2)}%`
      );
    }
    if (summary.averageAttempts > baseline.averageAttempts) {
      failures.push(
        `average attempts ${summary.averageAttempts.toFixed(1)} exceed baseline ${baseline.averageAttempts.toFixed(1)}`
      );
    }
