...
export class ZxingCppWasmEngineImpl implements ZxingCppWasmEngine {
  ...
  async decode(frame: NormalizedFrame, options: EngineDecodeOptions): Promise<EngineOutcome> {
    ...
    if (!options.formats.includes("qr_code")) return { ok: false, category: "unsupported-format", message: "ZXing-C++ WASM is publicly enabled for QR Code Model 2 only.", elapsedMs: 0 };
    const pixels = frame.width * frame.height;
    ...
    try {
      ...
      for (let index = 0; index < nativeCount; index += 1) {
        ...
        if (!["QRCode", "QRCodeModel2"].includes(result.format)) throw new ZxingCppWasmError("invalid_native_result", `Unexpected native format '${String(result.format)}'.`);
