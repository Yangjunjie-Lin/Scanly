...
const block = [
  ...
  `| Positive decode recall | **${Math.round(balanced.decodeRecall * balanced.positiveCases)}/${balanced.positiveCases} (${(balanced.decodeRecall * 100).toFixed(1)}%)** |`,
  `| Negative false positives | **${balanced.falsePositiveCount}/${balanced.negativeCases} (${(balanced.falsePositiveRate * 100).toFixed(1)}%)** |`,
  `| Remaining failure | ${balanced.remainingFailures.map((id) => `\`${id}\``).join(", ")} |`,
