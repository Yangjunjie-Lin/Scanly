...
- `benchmark:tracking`: at least 24 deterministic Beta 2 Ground Truth scenarios spanning basic, multi-object, identity, lifecycle, batch, and stress families. Reports derive identity switches, fragmentation, false tracks, matched/missed observations, recall/precision, false batch completion, and 1/4/8/16 target development baselines.

## Tracking reliability tiers

`npm run test:tracking:soak` processes 10,000 deterministic frames containing 16 moving tracks. It requires one bounded decode input set per frame, bounded track/lost/ROI/pending state, periodic global recovery, no retained event history, and zero controlled tracking state after disposal. It is labeled Core evidence and does not claim Worker execution.
