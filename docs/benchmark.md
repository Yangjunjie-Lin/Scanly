...
External open-license photographs provide third-party real-world validation but do not satisfy the project-owned photograph release gate. The separate `externalOpenLicenseRealWorld` cohort now passes blocking CI regression gates over 12/12 SHA-pinned photographs, 17 visible physical instances, 16/16 deduplicated `(format, payload, isGs1)` results, zero unexpected results, zero format/GS1 misclassifications, complete provenance, and public-repository safety. These regression gates never count as project-owned evidence. They do not activate `v2-alpha5-r1`; that identifier is a **historical abandoned release plan**. Alpha.5 will not be retroactively released, and the next eligible frozen evidence family is `v2-beta1-r1`.
