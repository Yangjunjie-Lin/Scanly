...
External open-license photographs provide third-party real-world validation but do not satisfy the project-owned photograph release gate. The separate `externalOpenLicenseRealWorld` cohort and its non-release corpus-count gate could not activate `v2-alpha5-r1`; that identifier is now a **historical abandoned release plan**. Alpha.5 will not be retroactively released, and the next eligible frozen evidence family is `v2-beta1-r1`.
