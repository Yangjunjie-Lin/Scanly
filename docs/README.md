# Scanly documentation

## Getting started

- [SDK usage for Browser, Node, React, and Core](sdk/usage.md)
- [Worker and WASM deployment](sdk/worker-deployment.md)
- [iOS getting started](native/ios-getting-started.md)
- [Android getting started](native/android-getting-started.md)

## API and capabilities

- [Public API](sdk/public-api.md)
- [API stability](sdk/api-stability.md)
- [Error catalog](sdk/error-catalog.md)
- [Cross-platform contract](sdk/cross-platform-contract.md)
- [Supported formats](symbologies.md)
- [Scenarios](scenarios/configuration.md)
- [Realtime scanning](beta1-realtime-runtime.md)
- [Tracking and batch](beta2-tracking-batch.md)
- [Industrial recovery](beta3-industrial-recovery.md)

## Native and compatibility

- [Native architecture](native/architecture.md)
- [Native memory model](native/memory-model.md)
- [Native compatibility](native/compatibility.md)
- [Platform compatibility matrix](platform-compatibility.md)

## Security, privacy, and maintenance

- [Security policy](../SECURITY.md)
- [Secure integration](security/secure-integration.md)
- [Threat model](security/threat-model.md)
- [Maintenance policy](maintenance.md)
- [Repository settings](maintenance/github-repository-settings.md)

## Migration, benchmarking, and release integrity

- [Migrate from v1 to v2.0.0](migration/v1-to-v2.md)
- [Post-release Physical Device Validation](physical-device-validation.md)
- [Benchmark methodology](benchmarking/methodology.md)
- [Benchmark provenance](benchmarking/provenance.md)
- [v2.0.0 release notes](releases/v2.0.0.md)
- [Stable qualification manifest](../release/stable/v2.0.0-manifest.json)
- [Stable publication record](../release/stable/v2.0.0-publication-record.json)
- [SDK v2 development history](history/sdk-v2-development-history.md)

Current physical validation status is tracked by
[Issue #13](https://github.com/Yangjunjie-Lin/Scanly/issues/13).
