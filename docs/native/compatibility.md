...
Scanly Native SDK v2.0.0 is a Stable software release. Automated build and fixture coverage is verified; post-release physical qualification remains pending.

| Target | Minimum / ABI | Distribution | Automated verification | Physical status |
| --- | --- | --- | --- | --- |
| iOS | iOS 13; Swift tools 5.9; current supported Xcode capable of Swift 5.9 packages | SPM source package at `native/ios` in tag `v2.0.0` | Swift build/test, simulator, unsigned device compilation | `POST_RELEASE_VALIDATION_PENDING` |
...
Distribution boundaries for v2.0.0:
...
- App Store, Google Play, React Native, and Flutter: not SDK distribution channels for v2.0.0.
