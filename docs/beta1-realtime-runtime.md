...
| --- | --- | --- |
| Scanner Core Soak | 10,000 successful observations through the scheduler/session with a deterministic fake decoder | Frame ownership, lifecycle, temporal confirmation, once-per-session repeat suppression, ROI-bearing follow-up requests, drained active/pending work, and final controlled state. It records `workerEvidence: "not-applicable"` and a Worker count of zero; zero is not a persistent-Worker pass. |
...
The dedicated Worker/WASM report is produced by `npm run test:scanner:worker-wasm-soak -- --iterations=1000`; `npm run test:scanner:worker-wasm-soak:extended` supplies the 10,000-frame extended tier. A pull request can run it against its exact head by applying the `scanner-extended-soak` label. The scheduled/manual workflow defaults to `develop/sdk-v2`; GitHub registers its cron and `workflow_dispatch` entry only after this workflow file is promoted to the repository default branch. By default, `npm run benchmark:realtime` evaluates the 20 semantic scenarios and Core soak and also requires a passing same-source Worker/WASM report. The integration-only CI producer uses an explicit `--allow-missing-worker-wasm` scope before the dedicated Worker artifact exists; the final Real-Time Benchmark requires a clean repository and exact-source Worker evidence with `--require-worker-wasm`.
