...
The 20-sequence harness uses scenario-specific drivers and independent Ground Truth; report schema `2.0-beta1` keeps expected events separate from decoder stimulus and records observed events, assertions, failure reasons, metrics, and timelines. Its 10,000-frame Scanner Core Soak uses a fake decoder and explicitly marks Worker evidence not applicable. Persistent browser Worker and ZXing-C++ WASM evidence comes only from the separate 1,000-frame pull-request soak or 10,000-frame scheduled/manual soak.
