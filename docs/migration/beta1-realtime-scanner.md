...
Browser automation covers real-time smoke, lifecycle, once-per-session repeat, and latest-frame backpressure in Chromium, Firefox, and WebKit. The separate curated open-license camera-photo gate covers Data Matrix, PDF417, Code 128, and EAN/UPC, but neither deterministic browser sources nor repository photos replace [Issue #13](https://github.com/Yangjunjie-Lin/Scanly/issues/13) or physical-device validation. Runtime integration may be `GO` after its gates pass; release remains `NO-GO` until independent device evidence exists. No `v2-beta1-r1` evidence activation is authorized, and no exact-SHA GitHub pass is implied by this migration guide.
