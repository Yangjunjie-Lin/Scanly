...
Beta 4 adds a separate `/device-lab` route. It shows browser and MediaTrack reports, exercises bounded camera lifecycle/capability APIs, records performance windows, observes external resource requests, and exports review-required drafts. The regular demo remains free of test protocol state. `/device-lab/targets` renders fixed screen targets generated from decoder-independent Ground Truth.
