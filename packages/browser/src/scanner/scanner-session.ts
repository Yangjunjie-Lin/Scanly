...
import { CameraRecoveryController, cameraError, type CameraRecoveryPolicy, type DeviceDiagnostics } from "./camera-platform.js";
...
export class ScannerSession {
  ...
  private readonly cameraRecovery: CameraRecoveryController;
  ...
  async start(): Promise<void> {
    ...
    try {
      await this.source.start(
        ...
        (event) => this.handleCameraLifecycle(event, lifecycleGeneration),
      );
  ...
  async stop(): Promise<void> {
    ...
    this.recoveryPromise = null; this.backgroundPaused = false;
  ...
  getDeviceDiagnostics(): DeviceDiagnostics | undefined { return this.source.getDeviceDiagnostics?.(); }
  getCameraCapabilities(): CameraCapabilities { return this.capabilityController?.getCapabilities() ?? { torch: false, focusMode: false }; }
  setTorch(enabled: boolean): Promise<CapabilityResult<boolean>> { return this.capabilityController?.setTorch(enabled) ?? Promise.resolve({ ok: false, error: sdkError("camera_capability_unsupported", "Torch is not available for this scanner source.") }); }
  setZoom(value: number, manual = true): Promise<CapabilityResult<number>> { return this.capabilityController?.setZoom(value, manual) ?? Promise.resolve({ ok: false, error: sdkError("camera_capability_unsupported", "Zoom is not available for this scanner source.") }); }
  requestFocus(): Promise<CapabilityResult<boolean>> { return this.capabilityController?.requestFocus() ?? Promise.resolve({ ok: false, error: sdkError("camera_capability_unsupported", "Focus is not available for this scanner source.") }); }
  ...
  private handleSourceEnded(lifecycleGeneration: number): void {
    queueMicrotask(() => {
      ...
      if (this.state === "starting" || !this.source.restart) { void this.stop(); return; }
  ...
  private handleCameraLifecycle(event: import("./camera-platform.js").CameraLifecycleEvent, lifecycleGeneration: number): void {
    if (lifecycleGeneration !== this.lifecycleGeneration || ["stopping", "stopped", "failed"].includes(this.state)) return;
    ...
    if (event.reason === "background-suspended") {
      ...
      if (this.state === "scanning" || this.state === "starting") {
        ...
        this.source.pause?.(); this.scheduler.pause(); this.backgroundPaused = true; this.setState("paused");
      }
      return;
    }
    if (event.reason === "foreground-resumed") {
      ...
      if (!track || track.readyState === "ended") { void this.recoverCamera("foreground-track-not-live", lifecycleGeneration); return; }
      this.source.resume?.(); this.scheduler.resume(); this.setState("scanning");
      return;
    }
    this.invalidateCameraGeneration(event.reason);
  }
  private invalidateCameraGeneration(reason: string): void {
    this.generation += 1; this.counters.cameraGenerationInvalidations += 1;
    this.activeDecodeController?.abort(); this.decoder.cancel();
    this.candidates.reset(); this.repeats.reset(); this.roi.reset(); this.trackingRuntime?.reset(); this.escalation.reset(); this.quality.reset();
    this.emitDiagnostic({ type: "camera", timestamp: Date.now(), detail: `scanner generation invalidated: ${reason}` });
  }
  private async recoverCamera(reason: string, lifecycleGeneration: number): Promise<void> {
    if (this.recoveryPromise) return this.recoveryPromise;
    const recovery = (async () => {
      const decision = this.cameraRecovery.next(reason);
      this.invalidateCameraGeneration(`recovery:${reason}`);
      this.source.pause?.(); this.scheduler.pause();
      ...
      if (lifecycleGeneration !== this.lifecycleGeneration || ["stopping", "stopped"].includes(this.state)) return;
      try {
        await this.source.restart();
        if (lifecycleGeneration !== this.lifecycleGeneration || ["stopping", "stopped"].includes(this.state)) return;
        this.cameraRecovery.markRestarted(); this.backgroundPaused = false; this.scheduler.resume(); this.setState("scanning");
        ...
        } else this.failCamera(typed, lifecycleGeneration);
      }
    })();
    this.recoveryPromise = recovery;
    try { await recovery; } finally { if (this.recoveryPromise === recovery) this.recoveryPromise = null; }
  }
  private failCamera(error: import("@scanly/core").SdkError, lifecycleGeneration: number): void {
    if (lifecycleGeneration !== this.lifecycleGeneration) return;
    this.lifecycleGeneration += 1; this.generation += 1; this.backgroundPaused = false;
