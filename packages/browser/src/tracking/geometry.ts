...
export function geometryCenter(geometry: BarcodeGeometry): Point {
  const box = geometry.boundingBox;
  return { x: box.x + box.width / 2, y: box.y + box.height / 2 };
}

export function geometryArea(geometry: BarcodeGeometry): number {
  return Math.max(EPSILON, Math.max(0, geometry.boundingBox.width) * Math.max(0, geometry.boundingBox.height));
}

export function geometryIoU(a: BarcodeGeometry, b: BarcodeGeometry): number {
...
export function geometryDistanceScale(a: BarcodeGeometry, b: BarcodeGeometry): number {
  const frameWidth = Math.max(a.frameWidth ?? 0, b.frameWidth ?? 0);
  const frameHeight = Math.max(a.frameHeight ?? 0, b.frameHeight ?? 0);
  if (frameWidth > 0 && frameHeight > 0) return Math.max(EPSILON, Math.hypot(frameWidth, frameHeight) * 0.25);
  const largestSymbolDiagonal = Math.max(
    Math.hypot(a.boundingBox.width, a.boundingBox.height),
    Math.hypot(b.boundingBox.width, b.boundingBox.height),
  );
  return Math.max(EPSILON, largestSymbolDiagonal * 4);
}

export function translateGeometry(geometry: BarcodeGeometry, x: number, y: number): BarcodeGeometry {
  return {
...
export function cloneGeometry(geometry: BarcodeGeometry): BarcodeGeometry {
  return {
...
export function isFiniteGeometry(geometry: BarcodeGeometry): boolean {
