...
export class TrackROISet {
    ...
    constructor(options = {}) {
        this.options = {
            maxCandidateTracks: boundedPositiveInteger(options.maxCandidateTracks, DEFAULT_MAX_CANDIDATE_TRACKS, HARD_MAX_CANDIDATE_TRACKS),
    ...
    plan(tracks, frame) {
        ...
        const uncoveredRegions = createUncoveredGrid(trackedROIs, this.options.uncoveredGridSize, this.options.maxUncoveredRegions);
        ...
        if (includeFullFrame)
            this.lastGlobalFrame = frame.frameId;
        this.lastSize = trackedROIs.length;
        const phases = [];
        if (trackedROIs.length > 0)
            phases.push("tracked-rois");
        if (uncoveredRegions.length > 0)
            phases.push("uncovered-regions");
        if (includeFullFrame)
            phases.push("full-frame");
        return { frameId: frame.frameId, trackedROIs, uncoveredRegions, includeFullFrame, phases };
    }
    get size() { return this.lastSize; }
    reset() {
        this.lastGlobalFrame = undefined;
        this.lastSize = 0;
    }
...
function createUncoveredGrid(tracked, gridSize, maximum) {
    if (tracked.length === 0)
        return [];
    ...
    for (let index = 0; index < cellChecks && uncovered.length < maximum; index += 1) {
        ...
        if (!tracked.some((roi) => rectanglesIntersect(cell, roi)))
            uncovered.push(cell);
    }
    return uncovered;
}
...
function statePriority(state) { return state === "confirmed" ? 0 : 1; }
...
//# sourceMappingURL=track-roi-set.js.map