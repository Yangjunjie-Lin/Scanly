...
export class BatchController {
    ...
    tracks = new Map();
    confirmedByPhysicalInstance = new Map();
    unexpectedByPhysicalInstance = new Map();
    duplicateByPhysicalInstance = new Map();
    ...
    constructor(options = {}) {
        ...
        this.maxRetainedTracks = validateRetentionLimit("maxRetainedTracks", options.maxRetainedTracks, DEFAULT_BATCH_MAX_RETAINED_TRACKS);
        this.maxRetainedPhysicalInstances = validateRetentionLimit("maxRetainedPhysicalInstances", options.maxRetainedPhysicalInstances, DEFAULT_BATCH_MAX_RETAINED_PHYSICAL_INSTANCES);
        ...
        if (requiredPhysicalInstances > this.maxRetainedPhysicalInstances) {
            throw new RangeError(`maxRetainedPhysicalInstances (${this.maxRetainedPhysicalInstances}) must cover the batch objective (${requiredPhysicalInstances}).`);
        }
    }
    getTracks() {
    ...
    applyTrackerUpdate(update) {
        ...
        for (const track of update.tracks) {
            ...
            if (track.state === "confirmed")
                this.confirmPhysicalTrack(track);
        }
     ...
     */
    applyTracks(tracks, timestamp = Date.now()) {
        if (this.status !== "collecting")
            return;
    ...
    fail(error) {
        if (this.status === "complete" || this.status === "failed" || this.status === "cancelled")
            return;
        this.status = "failed";
    ...
    reset() {
        ...
        for (const slot of this.expectedSlots)
            slot.physicalInstanceIds = [];
    ...
    upsertTrack(track) {
        ...
        if (existing)
            this.confirmedByPhysicalInstance.set(track.physicalInstanceId, cloneTrack(track));
    ...
    confirmPhysicalTrack(track) {
        const physicalInstanceId = track.physicalInstanceId.trim();
        if (!physicalInstanceId)
            throw new Error("A confirmed BarcodeTrack requires a physicalInstanceId.");
    ...
    evictOldestNonMatchedPhysicalInstance() {
        for (const physicalInstanceId of this.confirmedByPhysicalInstance.keys()) {
            if (!this.unexpectedByPhysicalInstance.has(physicalInstanceId)
                && !this.duplicateByPhysicalInstance.has(physicalInstanceId))
                continue;
    ...
    recordRetentionPeaks() {
        this.counters.peakRetainedTrackCount = Math.max(this.counters.peakRetainedTrackCount, this.tracks.size);
        this.counters.peakRetainedPhysicalInstanceCount = Math.max(this.counters.peakRetainedPhysicalInstanceCount, this.confirmedByPhysicalInstance.size);
        this.counters.peakUnexpectedQuantity = Math.max(this.counters.peakUnexpectedQuantity, this.unexpectedByPhysicalInstance.size);
        this.counters.peakDuplicateQuantity = Math.max(this.counters.peakDuplicateQuantity, this.duplicateByPhysicalInstance.size);
    }
    classifyChecklistTrack(track) {
    ...
    emitTrack(event) {
        ...
        if (counter === "trackaddedEvents")
            this.counters.trackAddedEvents += 1;
        else if (counter === "trackupdatedEvents")
            this.counters.trackUpdatedEvents += 1;
        else if (counter === "tracklostEvents")
            this.counters.trackLostEvents += 1;
        else if (counter === "trackrestoredEvents")
            this.counters.trackRestoredEvents += 1;
        else if (counter === "trackretiredEvents")
            this.counters.trackRetiredEvents += 1;
        for (const listener of this.trackListeners) {
            try {
                listener(event);
            }
            catch { /* observers do not own batch state */ }
        }
        this.emit(event);
    }
    emit(event) {
...
function validateExpectedCount(mode, value) {
    const required = mode === "expected-count";
    if (value === undefined) {
        if (required)
            throw new RangeError("expected-count mode requires expectedCount.");
        return undefined;
    }
...
function validateRetentionLimit(name, value, fallback) {
...
function validateExpectedItems(mode, expected) {
    if (mode !== "checklist")
        return [];
    if (!expected?.length)
        throw new RangeError("checklist mode requires at least one expected item.");
    return expected.map((item) => {
        if (!item.payload)
            throw new RangeError("Expected batch item payload must not be empty.");
        const quantity = item.quantity ?? 1;
        if (!Number.isInteger(quantity) || quantity <= 0)
            throw new RangeError("Expected batch item quantity must be a positive integer.");
