...
self.onmessage = async (event) => {
    ...
    try {
        ...
        const recovered = message.recovery
            ? await recoveryPipeline.run(frame, (candidate, request) => router.scan({ ...candidate, ownership: "borrowed", dispose: undefined }, { signal: request.signal, scenario: request.routeId === "general" ? message.scenario : createRecoveryProbeScenario(message.scenario, request.routeId) }), {
