export const SCENARIO_SCHEMA_VERSION = "2.1" as const;
...
export function validateScenario(input: unknown): ScenarioValidationResult {
  ...
  let candidate = input;
  const source = input as { schemaVersion?: unknown; ablation?: Record<string, unknown>; multiCode?: Record<string, unknown> };
  if (source.schemaVersion === "2.0") {
    ...
    if (typeof source.ablation?.zxingFallback === "boolean") {
      const migrated = JSON.parse(JSON.stringify(input)) as { schemaVersion: string; ablation: Record<string, unknown>; multiCode?: Record<string, unknown> };
      ...
      if (migrated.multiCode && migrated.multiCode.deduplication === undefined) migrated.multiCode.deduplication = "payload-format-spatial";
      candidate = migrated;
    }
  }
  const value = candidate as Partial<ScenarioDefinition>;
  ...
  for (const key of ["localization", "multiScale", "enhancement", "rotations", "multiEngineFallback", "splitImageFallback"] as const) booleanValue(value.ablation?.[key], `ablation.${key}`, issues);
  ...
  return { ok: true, value: candidate as ScenarioDefinition };
}
...
const MULTIFORMAT_FORMATS: BarcodeFormat[] = ["qr_code", "data_matrix", "pdf417", "code_128", "ean_13", "ean_8", "upc_a", "upc_e"];
const RETAIL_FORMATS: BarcodeFormat[] = ["ean_13", "ean_8", "upc_a", "upc_e", "code_128"];
const LOGISTICS_FORMATS: BarcodeFormat[] = ["code_128", "data_matrix", "qr_code"];
const DOCUMENT_FORMATS: BarcodeFormat[] = ["pdf417", "qr_code", "data_matrix"];
const FAST_SCENARIO: ScenarioDefinition = { ...cloneScenario(BASE_SCENARIO), id: "fast", localization: { ...BASE_SCENARIO.localization, maxCandidates: 2, cropPaddings: ["medium"], scales: [1] }, enhancement: { operators: ["contrast", "invert"], rotations: [0] }, multiCode: { enabled: false, maxResults: 1, deduplication: "payload-format-spatial" }, budgets: { ...BASE_SCENARIO.budgets, maxPixels: 4_000_000, maxCandidates: 2, maxAttempts: 18, maxIntermediateAllocations: 8, maxIntermediateBytes: 24 * 1024 * 1024, maxExecutionMs: 2_000 }, semanticParsers: ["url"] };
const ROBUST_SCENARIO: ScenarioDefinition = { ...cloneScenario(BASE_SCENARIO), id: "robust", localization: { ...BASE_SCENARIO.localization, maxCandidates: 8 }, multiCode: { enabled: true, maxResults: 12, deduplication: "payload-format-spatial" }, budgets: { ...BASE_SCENARIO.budgets, maxCandidates: 8, maxAttempts: 160, maxIntermediateAllocations: 40, maxIntermediateBytes: 96 * 1024 * 1024, maxExecutionMs: 20_000 } };

export const BUILTIN_SCENARIOS: Readonly<Record<"fast" | "balanced" | "robust" | "multiformat-balanced" | "retail-fast" | "logistics-balanced" | "document-robust", ScenarioDefinition>> = Object.freeze({
  fast: FAST_SCENARIO,
...
export function getBuiltinScenario(id: ScenarioPresetId): ScenarioDefinition {
