...
export class UrlSafetyClient implements UrlSafetyAnalyzer {
  private readonly cache: UrlSafetyCache;
  private readonly timeoutMs: number;
  constructor(private readonly options: { endpoint: string; cache?: UrlSafetyCache; timeoutMs?: number }) {
    if (!options.endpoint.startsWith("/") || options.endpoint.startsWith("//") || /[\\#]/.test(options.endpoint)) throw new Error("url_safety_endpoint_must_be_same_origin_path");
  ...
  analyze(input: string, options: AnalyzeOptions = {}): Promise<UrlSafetyAnalysis> {
    ...
    return this.cache.run(url, mode, async (signal) => {
      ...
      if (mode === "local-only") return new UrlRiskEngine().aggregate(local);
      return withBudget(async (boundedSignal) => {
        ...
        if (!response.ok || !response.headers.get("content-type")?.includes("application/json")) throw new Error("url_safety_endpoint_unavailable");
