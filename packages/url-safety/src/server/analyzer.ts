...
import type { AnalyzeOptions, LlmSafetyProvider, UrlReputationProvider, UrlSafetyAnalysis, UrlSafetyAnalyzer, UrlRiskEvidence } from "../types.js";
import { SafeRemoteFetcher, type RemoteFetcher } from "./fetcher.js";

export interface UrlSafetyPolicy { totalTimeoutMs?: number; reputationTimeoutMs?: number; remoteTimeoutMs?: number; aiTimeoutMs?: number }
export function createUrlSafetyAnalyzer(config: {
  ...
  if (providers.length > 16 || new Set(providers.map((provider) => provider.id)).size !== providers.length || providers.some((provider) => !/^[\w.-]{1,64}$/.test(provider.id))) throw new Error("invalid_reputation_provider_configuration");
  ...
  return {
    analyze(input: string, options: AnalyzeOptions = {}): Promise<UrlSafetyAnalysis> {
      ...
      return cache.run(local.normalizedUrl, mode, async (signal) => {
        ...
        if (mode === "local-only") return engine.aggregate(local);
        const evidence: Parameters<UrlRiskEngine["aggregate"]>[1] = { partial: false, networkRequested: true, privacy: { remoteAnalysisUsed: false, fullUrlShared: false, pageContentSharedWithLlm: false } };
        const original = new URL(local.normalizedUrl);
        // Embedded credentials never leave the process, regardless of mode.
        if (original.username || original.password) return engine.aggregate(local, { partial: true, remote: { status: "blocked", reason: "remote_inspection_credentials_blocked", redirectChain: [], bytesRead: 0 } });
        try {
          await withBudget(async (totalSignal) => {
            const reputation = async () => {
              const results: import("../types.js").UrlReputationResult[] = [];
              evidence.reputation = { status: "partial", results };
              await Promise.all(providers.map(async (provider) => {
                if (provider.configured === false) { results.push({ provider: provider.id, status: "not_configured", threats: [], operation: "lookup" }); return; }
                try {
                ...
                } catch { results.push({ provider: provider.id, status: "unavailable", threats: [], operation: "lookup" }); }
              }));
              ...
              if (!available.length || available.length < configured.length) evidence.partial = true;
            };
            const remote = async () => {
              if (mode === "reputation-only") return;
              try {
                evidence.privacy!.remoteAnalysisUsed = true;
              ...
              } catch { evidence.privacy!.fullUrlShared = true; evidence.remote = { status: "unavailable", reason: "remote_inspection_unavailable_or_timeout", redirectChain: [], bytesRead: 0 }; }
              if (evidence.remote.status !== "complete") evidence.partial = true;
            };
            await Promise.all([reputation(), remote()]);
            throwIfAborted(totalSignal);
            if (mode !== "ai-assisted") return;
            if (!config.llmProvider) { evidence.aiStatus = "not_configured"; evidence.partial = true; return; }
            const page = evidence.remote?.page;
            const { visibleTextSample = "", ...metadata } = page ?? {};
            const aiEvidence: UrlRiskEvidence = {
              ...
              remotePageMetadata: page ? metadata as UrlRiskEvidence["remotePageMetadata"] : undefined,
            ...
            const scrub = (value: unknown): unknown => {
              if (typeof value === "string") return secrets.reduce((text, secret) => text.split(secret).join("[REDACTED]"), sanitizeText(value)).slice(0, 20_000);
            ...
            try {
              ...
              const result = await withBudget((stageSignal) => config.llmProvider!.analyze(scrub(aiEvidence) as UrlRiskEvidence, { signal: stageSignal }), budgets.ai, totalSignal);
              const { provider, model, latencyMs, ...payload } = result;
              const validated = validateAi(payload);
              if (provider !== undefined && (typeof provider !== "string" || provider.length > 128) || model !== undefined && (typeof model !== "string" || model.length > 128) || latencyMs !== undefined && (!Number.isFinite(latencyMs) || latencyMs < 0)) throw new Error("ai_invalid_metadata");
            ...
            } catch { evidence.aiStatus = "unavailable"; evidence.partial = true; }
          }, budgets.total, signal);
        } catch { throwIfAborted(signal); evidence.partial = true; }
        throwIfAborted(signal);
        if (mode === "ai-assisted" && !evidence.aiStatus) { evidence.aiStatus = "unavailable"; evidence.partial = true; }
        if (evidence.reputation) evidence.reputation = { ...evidence.reputation, results: [...evidence.reputation.results].sort((a, b) => a.provider.localeCompare(b.provider)) };
