...
export async function urlSafetyCacheKey(url: string): Promise<string> {
...
interface Entry {
  ...
  promise: Promise<UrlSafetyAnalysis>;
...
export class UrlSafetyCache {
  ...
  async run(url: string, mode: UrlSafetyPrivacyMode, work: (signal: AbortSignal) => Promise<UrlSafetyAnalysis>, signal?: AbortSignal): Promise<UrlSafetyAnalysis> {
    throwIfAborted(signal);
    const key = `${mode}:${await urlSafetyCacheKey(url)}`;
    throwIfAborted(signal);
    for (const [id, cached] of this.entries) if (cached.settled && cached.expires <= Date.now()) this.entries.delete(id);
    let entry = this.entries.get(key);
    if (!entry) {
      if (this.entries.size >= this.maxEntries) {
        const oldest = [...this.entries].find(([, value]) => value.settled);
        if (!oldest) throw new Error("url_safety_cache_busy");
  ...
  clear(): void { for (const entry of this.entries.values()) entry.controller.abort(); this.entries.clear(); }
}
