...
export function isPublicInternetAddress(input) {
    if (!isIP(input) || input.includes("%"))
        return false;
    // Azure WireServer uses a public-looking virtual infrastructure address.
    if (input === "168.63.129.16")
        return false;
...
export async function resolvePublicTarget(url) {
    if (!["http:", "https:"].includes(url.protocol))
        throw new RemotePolicyError("remote_inspection_scheme_blocked");
    if (url.username || url.password)
        throw new RemotePolicyError("remote_inspection_credentials_blocked");
    if (url.port && url.port !== "80" && url.port !== "443")
        throw new RemotePolicyError("remote_inspection_port_blocked");
    const host = url.hostname.replace(/^\[|\]$/g, "").replace(/\.$/, "");
    if (!host.includes(".") && !isIP(host) || /(^|\.)(localhost|local|internal|lan|home|test|invalid)$/.test(host))
        throw new RemotePolicyError("remote_inspection_host_blocked");
    let addresses;
    if (isIP(host))
        addresses = [host];
    else {
        const resolve = async (family) => {
            try {
                return await (family === 4 ? resolve4(host) : resolve6(host));
            }
            catch (error) {
    ...
    if (!addresses.length)
        throw new Error("remote_inspection_dns_empty");
    if (!addresses.every(isPublicInternetAddress))
        throw new RemotePolicyError("remote_inspection_private_ip_blocked");
    return addresses[0];
}
