...
export class UrlRiskEngine {
    aggregate(local, evidence = {}) {
        const reasons = local.signals.map((signal) => ({ id: signal.id, source: "local", authority: signal.id === "private_network_target" || signal.id === "embedded_credentials" ? 2 : 4, description: signal.description }));
        ...
        if (page?.passwordInput) {
            score += 12;
            reasons.push({ id: "password_form", source: "remote", authority: 3, description: "Page contains a password input; verify the site's identity before entering credentials." });
        }
        if (page?.externalFormActions.length) {
            score += 12;
            reasons.push({ id: "external_form_action", source: "remote", authority: 3, description: "A form submits to a different hostname." });
        }
        if (evidence.remote?.status === "blocked") {
            reasons.push({ id: evidence.remote.reason ?? "remote_blocked", source: "policy", authority: 2, description: "Remote inspection was blocked by the network safety policy." });
        }
        if (evidence.ai) {
