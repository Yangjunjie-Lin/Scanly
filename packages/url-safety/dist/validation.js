...
const aiValidator = ajv.compile(AI_RISK_SCHEMA);
export function validateAi(value) {
    if (!aiValidator(value))
        throw new Error("ai_invalid_structured_output");
    return value;
}
...
const reputationValidator = ajv.compile(reputationResult);
export function validateReputation(value) {
    if (!reputationValidator(value))
        throw new Error("reputation_invalid_output");
    const result = value;
    if ((result.status === "match") !== (result.threats.length > 0))
        throw new Error("reputation_inconsistent_output");
    return result;
}
...
const analysisValidator = ajv.compile(object({
...
export function validateAnalysis(value) {
    if (!analysisValidator(value))
        throw new Error("url_safety_invalid_response");
    return value;
}
export async function readJsonBounded(response, maxBytes = 65_536, signal) {
    ...
    try {
        ...
        if (signal?.aborted)
            throw new Error("response_cancelled");
