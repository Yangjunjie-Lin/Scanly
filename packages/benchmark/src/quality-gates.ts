...
const HARD_ATTEMPT_LIMITS: Record<string, number> = { "14-damaged": 110, "16-multiple-codes": 30, "50-multiple-three": 70 };
...
export function evaluateBenchmarkGates(summary: BenchmarkRunSummary, baseline: BenchmarkBaseline, options: { fullSuite: boolean }): string[] {
  ...
  if (options.fullSuite) {
    ...
    if (summary.p95Attempts > Math.max(baseline.p95Attempts, 100)) failures.push(`P95 attempts ${summary.p95Attempts.toFixed(1)} exceed profile limit`);
    if (summary.averageMs > baseline.averageMs * TIMING_TOLERANCE) failures.push(`average latency exceeds ${TIMING_TOLERANCE}x baseline`);
    if (summary.medianMs > baseline.medianMs * TIMING_TOLERANCE) failures.push(`median latency exceeds ${TIMING_TOLERANCE}x baseline`);
    if (summary.p95Ms > baseline.p95Ms * TIMING_TOLERANCE) failures.push(`P95 latency exceeds ${TIMING_TOLERANCE}x baseline`);
  }
