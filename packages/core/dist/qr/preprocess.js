...
export function contrastStretch(src, budget) {
    ...
    for (let i = 0; i < data.length; i += 4) {
        if ((i & 0xffff) === 0)
            budget?.throwIfExceeded("contrast-analysis");
        const g = data[i];
        if (g < min)
            min = g;
        if (g > max)
    ...
    for (let i = 0; i < data.length; i += 4) {
        if ((i & 0xffff) === 0)
            budget?.throwIfExceeded("contrast-apply");
...
export function gammaCorrect(src, gamma = 0.7, budget) {
    const gray = toGrayscale(src, budget);
    const data = gray.data;
...
export function invertColors(src, budget) {
    const data = new Uint8ClampedArray(src.data);
    for (let i = 0; i < data.length; i += 4) {
        if ((i & 0xffff) === 0)
            budget?.throwIfExceeded("invert");
        data[i] = 255 - data[i];
...
export function fixedThreshold(src, threshold, budget) {
    const gray = toGrayscale(src, budget);
    const data = gray.data;
    const t = Math.max(0, Math.min(255, threshold));
    for (let i = 0; i < data.length; i += 4) {
        if ((i & 0xffff) === 0)
            budget?.throwIfExceeded("threshold");
...
export function computeOtsuThreshold(src, budget) {
    const hist = new Array(256).fill(0);
    const data = src.data;
    let total = 0;
    for (let i = 0; i < data.length; i += 4) {
        if ((i & 0xffff) === 0)
            budget?.throwIfExceeded("otsu-histogram");
...
export function sharpen(src, budget) {
    ...
    for (let y = 1; y < height - 1; y++) {
        if ((y & 31) === 0)
            budget?.throwIfExceeded("sharpen");
...
export function applyPreprocess(src, method, budget) {
    switch (method) {
        case "original":
