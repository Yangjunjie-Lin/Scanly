...
export class BarcodeDifficultyAnalyzer {
    ...
    analyze(frame, runtime = {}) {
        ...
        let inkTop = rows;
        let inkBottom = 0;
        let horizontalBorderInk = 0;
        let horizontalBorderSamples = 0;
        let verticalBorderInk = 0;
        let verticalBorderSamples = 0;
        ...
        for (let row = 0; row < rows; row += 1) {
            ...
            for (let column = 0; column < columns; column += 1) {
                ...
                if (value <= 7)
                    clippedLow += 1;
                if (row === 0 || row === rows - 1) {
                    horizontalBorderSamples += 1;
                    if (value < 180)
                        horizontalBorderInk += 1;
                }
                if (column === 0 || column === columns - 1) {
                    verticalBorderSamples += 1;
                    if (value < 180)
                        verticalBorderInk += 1;
                }
                if (value < 180) {
