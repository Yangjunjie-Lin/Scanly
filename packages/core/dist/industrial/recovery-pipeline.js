...
export function createRecoveryProbeScenario(source, routeId) {
    ...
    async run(frame, decode, options = {}) {
        ...
        try {
            ...
            if (normal.ok) {
                ...
                const snapshot = { difficulty: diagnosis, routesAttempted: [], routeSucceeded: "general", attemptCount: 0, processedPixels: 0, candidateConflictCount: candidateSet.conflicts.length, evidence, insufficientEvidence: false, reasons: [] };
            ...
            const snapshot = { difficulty: diagnosis, routesAttempted: routes.map((route) => route.route), ...(successfulRoute ? { routeSucceeded: successfulRoute } : {}), attemptCount: recoveryAttempts, processedPixels, candidateConflictCount: candidateSet.conflicts.length, evidence, insufficientEvidence: false, reasons: [...reasons] };
...
function attachEvidence(outcome, evidence, route, frameId, snapshot) {
    const results = outcome.results.map((result) => attachResultEvidence(result, evidence, route, frameId, snapshot));
    return { ...outcome, results, primary: results[0], frameId };
}
function attachResultEvidence(result, evidence, route, frameId, snapshot) { return { ...result, frameId, metadata: { ...result.metadata, recoveryRoute: route, scanEvidence: evidence, scannerDiagnostics: snapshot, evidenceScoreIsCalibratedProbability: false } }; }
