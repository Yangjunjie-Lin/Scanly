export function geometryEvidence(candidate) {
    const points = candidate.geometry;
    if (!points || points.length < 3 || points.some((point) => !Number.isFinite(point.x) || !Number.isFinite(point.y)))
        return 0;
    let area = 0;
...
export function buildScanEvidence(candidates, temporalObservations = 1) {
    ...
    const checksumValues = candidates.map((candidate) => candidate.validation.checksumValidated).filter((value) => value !== undefined);
...
export function decodeCandidateFromResult(result, route, elapsedMs) {
...
//# sourceMappingURL=scan-evidence.js.map