import { createRgbaFrame } from "../contracts/frame.js";
import { affineRecoveryTransform, identityRecoveryTransform } from "./coordinate-transform.js";
let candidateSequence = 0;
export function originalRecoveryCandidate(frame) {
    return {
...
export function createRecoveryCandidate(source, routeId, data, width, height, transform, memory, kind, diagnostics) {
...
export function localContrastNormalize(data, width, height) {
    const gray = grayscale(data);
    const tilesX = Math.min(8, Math.max(1, width));
    const tilesY = Math.min(8, Math.max(1, height));
    const minimums = new Uint8Array(tilesX * tilesY);
    const maximums = new Uint8Array(tilesX * tilesY);
    minimums.fill(255);
    for (let y = 0; y < height; y += 1)
        for (let x = 0; x < width; x += 1) {
            const tileX = Math.min(tilesX - 1, Math.floor(x * tilesX / width));
            const tileY = Math.min(tilesY - 1, Math.floor(y * tilesY / height));
            const tile = tileY * tilesX + tileX;
            const value = gray[(y * width + x) * 4];
            minimums[tile] = Math.min(minimums[tile], value);
            maximums[tile] = Math.max(maximums[tile], value);
        }
    ...
    for (let y = 0; y < height; y += 1)
        for (let x = 0; x < width; x += 1) {
            ...
            const tileY = Math.min(tilesY - 1, Math.floor(y * tilesY / height));
            const tile = tileY * tilesX + tileX;
            const low = minimums[tile];
            const high = maximums[tile];
            const value = gray[(y * width + x) * 4];
            const normalized = high - low < 8 ? value : clampByte((value - low) * 255 / (high - low));
...
export function claheLike(data, width, height) {
    const normalized = localContrastNormalize(data, width, height);
    const histogram = new Uint32Array(256);
    for (let index = 0; index < normalized.length; index += 4)
        histogram[normalized[index]] += 1;
    const cap = Math.max(1, Math.floor(width * height / 128));
    let excess = 0;
    for (let index = 0; index < histogram.length; index += 1)
        if (histogram[index] > cap) {
            excess += histogram[index] - cap;
            histogram[index] = cap;
        }
    const spread = Math.floor(excess / 256);
    for (let index = 0; index < histogram.length; index += 1)
        histogram[index] += spread;
    const map = new Uint8Array(256);
    let cumulative = 0;
    const total = Math.max(1, histogram.reduce((sum, value) => sum + value, 0));
    for (let index = 0; index < 256; index += 1) {
        cumulative += histogram[index];
        map[index] = clampByte(cumulative * 255 / total);
    }
    for (let index = 0; index < normalized.length; index += 4)
        normalized[index] = normalized[index + 1] = normalized[index + 2] = map[normalized[index]];
    return normalized;
}
export function adaptiveThreshold(data, width, height, radius = 8, offset = 7) {
    const gray = grayscale(data);
    const integral = new Uint32Array((width + 1) * (height + 1));
    for (let y = 1; y <= height; y += 1) {
        let rowSum = 0;
        for (let x = 1; x <= width; x += 1) {
            rowSum += gray[((y - 1) * width + x - 1) * 4];
...
export function illuminationNormalize(data, width, height) {
    ...
    for (let y = 0; y < height; y += 1)
        for (let x = 0; x < width; x += 1) {
            ...
            means[cell] += gray[(y * width + x) * 4];
            counts[cell] += 1;
        }
    for (let index = 0; index < means.length; index += 1)
        means[index] /= Math.max(1, counts[index]);
    for (let y = 0; y < height; y += 1)
        for (let x = 0; x < width; x += 1) {
            const gx = Math.min(grid - 1, Math.floor(x * grid / width));
            const gy = Math.min(grid - 1, Math.floor(y * grid / height));
            const background = means[gy * grid + gx];
            const index = (y * width + x) * 4;
            const value = clampByte(gray[index] - background + 150);
            gray[index] = gray[index + 1] = gray[index + 2] = value;
        }
    return gray;
}
export function unsharpMask(data, width, height, amount = 1.15, directional = "both") {
    const gray = grayscale(data);
    ...
    for (let y = 1; y < height - 1; y += 1)
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const blurred = neighbours.reduce((sum, value) => sum + value, 0) / neighbours.length;
            const value = clampByte(center + (center - blurred) * amount);
            output[index] = output[index + 1] = output[index + 2] = value;
        }
    return output;
}
export function glareAlternative(data, width, height) {
    const gray = grayscale(data);
    let clipped = 0;
    for (let index = 0; index < gray.length; index += 4) {
        if (gray[index] >= 248) {
            clipped += 1;
            gray[index] = gray[index + 1] = gray[index + 2] = 255;
        }
    }
    return { data: adaptiveThreshold(gray, width, height, 10, 3), glareRatio: clipped / Math.max(1, gray.length / 4) };
}
export function resizeNearest(data, width, height, factor) {
...
export function resizeBilinear(data, width, height, factor) {
    const nextWidth = Math.max(1, Math.round(width * factor));
    const nextHeight = Math.max(1, Math.round(height * factor));
    const output = new Uint8ClampedArray(nextWidth * nextHeight * 4);
    for (let y = 0; y < nextHeight; y += 1)
...
export function morphologicalGradient(data, width, height) {
    ...
    const low = erode(source, width, height);
    const output = new Uint8ClampedArray(source.length);
    for (let index = 0; index < output.length; index += 4) {
        const value = high[index] - low[index];
        output[index] = output[index + 1] = output[index + 2] = value;
        output[index + 3] = 255;
    }
    return output;
}
export function padNeutral(data, width, height, padding) {
    const nextWidth = width + padding * 2;
    const nextHeight = height + padding * 2;
    const output = new Uint8ClampedArray(nextWidth * nextHeight * 4);
    output.fill(255);
    for (let y = 0; y < height; y += 1)
        output.set(data.subarray(y * width * 4, (y + 1) * width * 4), ((y + padding) * nextWidth + padding) * 4);
...
function dilate(data, width, height) { return morphology(data, width, height, Math.max); }
function erode(data, width, height) { return morphology(data, width, height, Math.min); }
function morphology(data, width, height, reducer) {
    const output = new Uint8ClampedArray(data.length);
    for (let y = 0; y < height; y += 1)
        for (let x = 0; x < width; x += 1) {
            const values = [];
            for (let dy = -1; dy <= 1; dy += 1)
                for (let dx = -1; dx <= 1; dx += 1) {
                    ...
                    values.push(data[(sy * width + sx) * 4]);
                }
            const index = (y * width + x) * 4;
...
export function writeSample(target, targetIndex, source, width, height, sx, sy) {
    ...
    for (let channel = 0; channel < 3; channel += 1) {
        ...
        target[targetIndex + channel] = clampByte(top * (1 - dy) + bottom * dy);
    }
