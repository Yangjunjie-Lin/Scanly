...
export class CandidateRegionDetector {
  constructor(private readonly options: CandidateRegionDetectorOptions = {}) {}
  ...
  detect(frame: NormalizedFrame): BarcodeCandidateRegion[] {
    ...
    const cells = new Array<{ edge: number; horizontal: number; vertical: number; contrast: number }>(grid * grid);
    ...
    const raw: BarcodeCandidateRegion[] = [];
    for (let gy = 0; gy <= grid - windowCells; gy += 1) for (let gx = 0; gx <= grid - windowCells; gx += 1) {
      let edge = 0; let horizontal = 0; let vertical = 0; let contrast = 0;
      for (let wy = 0; wy < windowCells; wy += 1) for (let wx = 0; wx < windowCells; wx += 1) {
        const cell = cells[(gy + wy) * grid + gx + wx];
        edge += cell.edge; horizontal += cell.horizontal; vertical += cell.vertical; contrast += cell.contrast;
      }
      ...
      if (score < (this.options.minimumScore ?? 0.16)) continue;
      const x = gx * cellWidth; const y = gy * cellHeight;
...
function inspectCell(frame: NormalizedFrame, x0: number, y0: number, width: number, height: number) {
  const step = Math.max(1, Math.floor(Math.min(width, height) / 8));
  let edge = 0; let horizontal = 0; let vertical = 0; let minimum = 255; let maximum = 0; let count = 0;
  for (let y = y0 + step; y < Math.min(frame.height, y0 + height); y += step) for (let x = x0 + step; x < Math.min(frame.width, x0 + width); x += step) {
    ...
    horizontal += dx; vertical += dy; edge += Math.min(1, (dx + dy) / 96);
    minimum = Math.min(minimum, value); maximum = Math.max(maximum, value); count += 1;
  }
  return { edge: edge / Math.max(1, count), horizontal, vertical, contrast: (maximum - minimum) / 255 };
}

function nonMaximumSuppression(regions: BarcodeCandidateRegion[], maximum: number): BarcodeCandidateRegion[] {
  const sorted = [...regions].sort((a, b) => b.score - a.score || a.id.localeCompare(b.id));
  const kept: BarcodeCandidateRegion[] = [];
  for (const region of sorted) {
    if (kept.length >= maximum) break;
    if (!kept.some((existing) => intersectionOverUnion(existing.boundingBox, region.boundingBox) > 0.45)) kept.push(region);
  }
  return kept;
}

function intersectionOverUnion(a: BarcodeCandidateRegion["boundingBox"], b: BarcodeCandidateRegion["boundingBox"]): number {
...
function luminance(frame: NormalizedFrame, x: number, y: number): number {
